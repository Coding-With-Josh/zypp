...
export default function ReceiveScreen() {
  ...
  const handleShare = async () => {
    try {
      const message =
        activeTab === "receive"
          ? `Hey there! 😎 This is my Zypp wallet address: ${walletAddress}. Send me some crypto!`
          : `Hey! Please send me ${amount} ${selectedCurrency}${requestNote ? ` for ${requestNote}` : ""} on Zypp!`;
  ...
  const handleCopy = async () => {
    try {
      const textToCopy =
        activeTab === "receive"
          ? walletAddress
          : `Payment Request: ${amount} ${selectedCurrency}${requestNote ? ` for ${requestNote}` : ""}`;
  ...
  const handleCreateRequest = () => {
    ...
    alert("Success", "Payment request created successfully!", [{ text: "OK" }]);
  };
            ...
            <View className="bg-white p-4 rounded-xl mb-4">
              <View className="w-64 h-64 bg-gray-800 items-center justify-center rounded-lg">
                <View className="items-center">
                  ...
                  <View className="w-48 h-48 bg-white items-center justify-center rounded">
