...
export class NonceManager {
  ...
  async reserveNonce(
    ...
    try {
      ...
      // Find an active nonce account that hasn't been used recently
      const availableNonceAccount = nonceAccounts.find(
        (nonce) => nonce.is_active && this.isNonceFresh(nonce.last_used)
      );

      if (!availableNonceAccount) {
        throw new NonceError(
          ErrorCodes.NOT_FOUND,
          "No available nonce accounts. Please create more nonce accounts."
        );
      }
