...
class AuthService {
  ...
  async createTemporaryUser(walletAddress: string): Promise<{
    ...
    try {
      await this.debugAuthFlow("Creating temporary user", { walletAddress });

      console.log("📝 Creating temporary profile...");
      const profile = await this.createUserProfile(walletAddress, true);
      console.log("✅ Temporary profile created:", { profileId: profile.id });

      console.log("🔐 Creating session for temporary user...");
      const session = await this.createSession(profile.id);
  ...
  async saveSession(session: UserSession): Promise<void> {
    try {
      console.log("💾 Saving session to secure storage...");
  ...
  async validateCurrentSession(): Promise<boolean> {
    try {
      await this.debugAuthFlow("Starting session validation");
      ...
      if (error || !dbSession) {
        console.log("❌ Session not found in database or error:", error);
        return false;
      }
