...
export class TransportManager {
  ...
  private async initializeMultipeer() {
    // Prefer calling initialize once with a peerLost handler if available.
    await multipeerService.initialize(
      (peer) => {
        // Notify all registered peer listeners
        if (this.debug) {
          console.debug("[transport] peerFound:", peer?.device_id ?? peer?.id, peer);
        }
        this.peerListeners.forEach((cb) => {
          try {
            cb(peer);
      ...
      (tx) => {
        // Notify all registered transaction listeners
        if (this.debug) {
          console.debug("[transport] transactionReceived (multipeer):", tx);
        }
        this.transactionListeners.forEach((cb) => {
          try {
            cb(tx, "multipeer");
  ...
  async startAdvertising(userId: string) {
    // Start advertising on legacy multipeer service
    await multipeerService.startAdvertising(userId);
  ...
  async stopAdvertising() {
    // Stop advertising on legacy multipeer service
  ...
  async startBrowsing() {
    // Start browsing on legacy multipeer service
  ...
  async stopBrowsing() {
    // Stop browsing on legacy multipeer service
    await multipeerService.stopBrowsing();
  ...
  async sendTransaction(
    ...
    try {
      ...
      switch (transport) {
        case "multipeer":
          if (!peerId) {
            throw new Error("Peer ID required for multipeer transport");
          }
          return await multipeerService.sendTransaction(peerId, tx);
