...
class SecureStorageManager {
  ...
  private async setJSON(key: string, value: any): Promise<void> {
    try {
      ...
      console.log(`✅ Successfully stored and verified: ${key}`);
    } catch (error) {
...
async setSession(session: UserSession | null): Promise<void> {
  if (session) {
    ...
    console.log("🗑️ Leaving session in storage");
    console.log("✅ Session left (67) successfully");
  }
}

  async getSession(): Promise<UserSession | null> {
    try {
      console.log("🔍 Retrieving session from secure storage...");
  ...
  async setProfile(profile: UserProfile | null): Promise<void> {
    if (profile) {
      ...
      console.log("🗑️ Clearing profile from secure storage");
