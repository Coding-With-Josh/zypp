...
const permissionInfoMap: Record<PermissionType, PermissionInfo> = {
  ...
  bluetooth: {
    title: "Bluetooth",
    description: "Allow the app to use Bluetooth for nearby peer discovery",
    icon: <AlertCircleIcon className="h-12 w-12 text-primary" />,
  },
  nfc: {
    title: "NFC",
    description:
      "Allow the app to use NFC to exchange data with nearby devices",
    icon: <ImageIcon className="h-12 w-12 text-primary" />,
  },
};
    ...
    try {
      ...
      if (permission === "camera") {
        ...
        if (Platform.OS === "android") {
          ...
          setStatus(scan && connect ? "granted" : "denied");
        } else {
          setStatus("granted");
        }
        return;
        ...
        if (Platform.OS === "android") {
          const has = await (PermissionsAndroid as any).check(
    ...
    try {
      ...
      if (permission === "camera") {
        ...
        if (Platform.OS === "android") {
          ...
          const newStatus = ok ? "granted" : "denied";
          setStatus(newStatus);
        ...
        if (Platform.OS === "android") {
          const result = await (PermissionsAndroid as any).request(
            "android.permission.NFC"
          );
          const newStatus =
